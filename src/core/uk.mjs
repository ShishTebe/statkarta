...
export function parseQualification(text) {
  ...
  // следующий пункт можно отделить запятой, союзом «и» или пробелом («пп. а д»); буква с точкой
  // («ч.», «п.») пунктом не считается – замечание от 23.09.2026
  const PT_NEXT = '(?:«[^»]{1,4}»|"[^"]{1,4}"|[а-яa-z](?![\\p{L}\\d.]))';
  const PT_SEP = '(?:\\s*,\\s*и\\s+|\\s*,\\s*|\\s+и\\s+|\\s+)';
  const rx = new RegExp(`(?:(пп?\\.|пункт[а-я]*)\\s*(${PT}(?:${PT_SEP}${PT_NEXT})*)\\s*)?(?:(чч?\\.|част[а-я]*)\\s*(\\d+(?:\\.\\d+)?(?:\\s*(?:,|и)\\s*\\d+(?:\\.\\d+)?)*)\\s*)?(?:ст\\.|стат[а-я]*)\\s*(\\d+(?:\\.\\d+)?)`, 'giu');
  ...
  segments.forEach((seg, i) => {
    ...
    rx.lastIndex = 0;
    while ((m = rx.exec(seg))) {
      // пункты пишут через запятую и через пробел: «а, д» и «а д» – это два пункта,
      // а «е.1» – один пункт с примечанием (замечание от 23.09.2026)
      const points = m[2] ? [...m[2].matchAll(/[«"]([^»"]+)[»"]|(?:^|[,\s])([а-яa-z])(?![\p{L}\d])/giu)]
        .flatMap((x) => String(x[1] ?? x[2] ?? '').split(/[\s,;]+/)).filter(Boolean) : [];
